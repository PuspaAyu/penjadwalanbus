  ...
  <div class = "panel-body">
    <table class="table table-bordered">
      ...
      <?php $i=0; foreach ($tempviewjadwal as $tempviewjadwal):?>  
        ...
        <tr>
          <form method="post" action="<?= Url::to(['karcis/save', 'id' => $tempviewjadwal['id_karcis']]); ?>">
          ...
          <td>
            <?= $form->field($model, 'pergi_awal')->textInput(['style'=>'width:70px']) ?>
          </td>
          <td>
           <?= $form->field($model, 'pergi_akhir')->textInput(['style'=>'width:70px']) ?>
          </td>
          <td>
            <?= $form->field($model, 'pulang_awal')->textInput(['style'=>'width:70px']) ?>            
          </td>
          <td>
           <?= $form->field($model, 'pulang_akhir')->textInput(['style'=>'width:70px']) ?>
          </td>
          <td>
